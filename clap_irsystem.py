...
class CombinedCLAPIRSystem(IRSystem):
    ...
    def __init__(self, tracks):
        ...
        valid_tracks = [
            ...
            if track.clap_tags_vector is not None and track.clap_vector is not None
        ]
        self.tracks = valid_tracks
        
        # Combine embeddings by averaging CLAP text and audio embeddings
        self.embedding_matrix = np.vstack([
            self.combine_embeddings(track.clap_tags_vector, track.clap_vector)
    ...
    def query(self, query: Track, n=10):
        ...
        if query.clap_tags_vector is None or query.clap_vector is None:
            raise ValueError("Query track does not have both CLAP text and audio embeddings")
        
        # Combine query embeddings
        query_vector = self.combine_embeddings(query.clap_tags_vector, query.clap_vector)
